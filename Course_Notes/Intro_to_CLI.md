...
- You need to use a TERMINAL EMULATOR (Example: _PuTTy_ is a popular choice)
- Connect via "Serial" (default settings)
...
Terminal appearance in ```User EXEC``` mode:

```(Hostname)>{command_here}```
...
Terminal appearance in ```Privileged EXEC``` mode: ```(Hostname)#{command_here}```
...
```Router#configure terminal``` or ```Router#conf t```
...
// enable secret will ALWAYS be hashed (at level 5 - MD5 hashing algorithm)
...
Type ```7``` is incredibly easy to crack--avoid using it.
...
**CORRECTION (last command in the above photo):
  ...
  - They should be entered in ```privileged EXEC``` mode, NOT ```Global Configuration``` mode. 
